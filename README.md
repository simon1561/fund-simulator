...
- `现金影响USD`：**可选的「到账金额手动覆盖」**，正数表示现金增加、负数表示减少。买入/卖出/分红留空时脚本会按 `数量 × 成交价 × USD汇率` 自动算；只有要覆盖自动结果，或表达没有「股数 × 单价」结构的现金流（如 `冲正`）时才填。
- `外部现金流USD`：只给 `入金` / `出金` 使用；入金正数，出金可以填正数或负数，脚本会按出金处理成负数。
- `标的币种`：自动从 `投资标的库` 的证券记录带出，例如腾讯会显示 `HKD`，不需要手填。
- `USD汇率`：目标货币折算成 USD 的汇率，例如 HKD 标的填 HKD->USD。手填值优先；买入/卖出/分红时如果留空，脚本会按 `交易日期` 自动抓当天或最近前一个可用交易日的汇率并回填。USD 标的留空时按 `1`。
- 买入股票：填 `数量` 和 `成交价`（原币）。`现金影响USD` 为空时脚本用 `数量 × 成交价 × USD汇率` 自动计算现金流出。
- 卖出股票：填 `数量` 和 `成交价`（原币）。`现金影响USD` 为空时脚本用 `数量 × 成交价 × USD汇率` 自动计算现金流入。
- 分红：和卖出同口径——`数量` 填应分红股数，`成交价` 填每股股息（原币，例如茅台每股 `28.02423` 元）。脚本按 `数量 × 成交价 × USD汇率` 算到账分红，`USD汇率` 留空按交易日自动抓。不填 `外部现金流USD`；`现金影响USD` 只在需要手动覆盖时填。
